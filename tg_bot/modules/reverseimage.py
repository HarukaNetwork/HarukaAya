...
def reverse(bot: Bot, update: Update, args: List[str]):
    ...
    try:
        ...
        if response != 400:
            await msg.edit_text("Image was successfully uploaded to Google."
                                "\n`Parsing source now. Maybe.")
        else:
            await msg.edit_text("Google told me to go away.")
            return
        ...
        if guess and imgspage:
            await msg.edit_text(f"[{guess}]({fetchUrl})\nLooking for images...")
        else:
            await msg.edit_text("Couldn't find anything.")
            return
